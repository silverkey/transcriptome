...
open(OUT,">$out_table");
print OUT "seqid\tlength\tA\tT\tC\tG\tGC\tGC_obsex_ratio\n";
...
while(my $seq = $in->next_seq) {
  ...
  # GC OBS/EX
  my $gc_freq = gc_freq($seq->seq,'GC');
  my $obsex = $gc_freq/(($c/$length)*($g/$length));

  print OUT join("\t",$seq->id,$length,$a,$t,$c,$g,$gc,$obsex)."\n";
