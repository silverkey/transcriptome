...
my $file1 =           'pm_transcriptome_2013_10_filtered.fasta';
my $file2 =           'sr_transcriptome_2013_10_filtered.fasta';
my $folder =          'pm_vs_sr_tblastx';
my $coverage_cutoff = 30;
my $algo =            'tblastx';
my $num_threads =     20;
my $overhang =        0.1;
my $evalue =          0.01;
...
mkdir($folder) unless $ONLY_BUILD_BRH;
chdir($folder);
open(LOG,">>LOG") or die "\nERROR opening LOG file: $!\n\n";

unless($ONLY_BUILD_BRH) {
  system("cp ../$file1 .");
  system("cp ../$file2 .");
  system("cp ../$0 .");
  ...
  run_blastplus($algo,$file1,$file2,$out1);
  run_blastplus($algo,$file2,$file1,$out2);
}
