library("edgeR")
counts = read.table('COUNTS.txt',row.names=1,head=T)
...
res = tt$table
sig = res[res$FDR<=0.01 & abs(res$logFC)>=log2(2),]
sig2 = merge(sig,cpm,by=0,all.x=T,sort=F)
write.table(sig2,file='significant.txt',sep='\t',quote=F)

res2 = merge(res,cpm,by=0,all.x=T,sort=F)
write.table(res2,file='all_statistics.txt',sep='\t',quote=F)
...
write.table(res3,file='all_statistics_annotated.txt',sep='\t',quote=F)

sig3 = merge(sig2,anno,by.x='Row.names',by.y='CompName',sort=F)
write.table(sig3,file='significant_statistics_annotated.txt',sep='\t',quote=F)
...
write.table(hsigp,file='higly_significant_plus_statistics_annotated.txt',sep='\t',quote=F)
write.table(hsigm,file='higly_significant_minus_statistics_annotated.txt',sep='\t',quote=F)
...
write.table(gtab,file='on_genome_statistics_annotated.txt',sep='\t',quote=F)
